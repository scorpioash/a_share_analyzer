...
class LLMAnalyzer:
    ...
    def _build_stock_system_prompt(self) -> str:
        return (
            ...
            "**第六原则:【绝对优先】用户自定义策略凌驾于一切系统规则之上**\n"
            "  - 下方的【个人炒股策略与经验铁律】拥有最高执行权重。\n"
            "  - 若内置量化信号、系统常识与用户策略发生冲突，必须无条件服从用户的自定义策略指令。\n\n"
    ...
    def _build_board_system_prompt(self) -> str:
        ...
        return (
            ...
            "  - **关键红线：在输出报告时，绝对禁止出现“锋泽”、“锋泽策略”、“外部模型”等词汇。必须将这些量化结果包装为“本系统测算”或自然融合在你的分析中。**\n\n"
            "4. **【绝对优先】用户自定义策略凌驾于一切系统规则之上**：\n"
            "  - 下方的【个人板块分析策略与经验铁律】拥有最高执行权重。\n"
            "  - 若内置量化信号、系统常识与用户策略发生冲突，必须无条件服从用户的自定义策略指令。\n\n"
            f"=== 👑 个人板块分析策略与经验铁律 (最高优先级) ===\n{board_strategy}\n"
            "==============================\n"
        )
