...
class EastMoneyDirectAPI:
    ...
    def _build_session(self, proxies=None) -> requests.Session:
        s = requests.Session()
        s.headers.update(self.DEFAULT_HEADERS)
        retry = Retry(
            total=self.max_retries, connect=self.max_retries, read=0,
            backoff_factor=0.5, status_forcelist=[], allowed_methods=['GET'],
        )
    ...
    def _get(self, host, path, params, session):
    ...
    def _try_multi_hosts(self, hosts, path, params, proxies=None, diag=None):
        last_err = None
        ...
        if diag:
            ...
            diag(f"EastMoney 所有节点失败 ({err_brief}) —— 已自动降级", "DEBUG")
    ...
    def get_intraday(self, code: str, proxies=None, diag=None) -> pd.DataFrame:
        ...
        data = self._try_multi_hosts(self.PUSH_HIS_HOSTS, '/api/qt/stock/trends2/get',
                                      params, proxies=proxies, diag=diag)
        d = data.get('data') or {}
...
class ClashHelper:
    EASTMONEY_DOMAINS = [
        ...
        '82.push2.eastmoney.com', '72.push2.eastmoney.com',
        '78.push2.eastmoney.com', 'quote.eastmoney.com',
    ]
    COMMON_API_URLS = [
        'http://127.0.0.1:9090',
        'http://127.0.0.1:9097',
...
def bypass_proxy(enable_dns_hijack: bool = True, mode: str = "auto"):
    """代理管理上下文。mode: bypass/use/keep/auto"""
    if mode == "auto":
...
class AShareDataFetcher:
    def __init__(self):
        ...
        # ⭐ 雪球 session — 行情中心/个股快照降级用
        # 必须先访问主页拿 cookie,否则雪球接口会返回 400
        self.xq_session = requests.Session()
        self.xq_session.headers.update({
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 '
        ...
        try:
            # 主页握手,拿 cookie (xq_a_token 等)
            self.xq_session.get('https://xueqiu.com/', timeout=3)
        except Exception:
            pass

        # 通达信直连引擎 (可选,依赖 tdx_provider.py)
        try:
            from tdx_provider import TDXProvider
            self.tdx = TDXProvider()
        except Exception:
            self.tdx = None
    ...
    def _diag(self, msg: str, level: str = "INFO"):
        ...
        level_fn = {
            'ERROR': logger.error, 'WARN': logger.warning,
    ...
    # ==================================================================
    # 核心：实时快照 —— 多源融合 (通达信 -> 东财直连 -> 雪球 -> 新浪 -> 腾讯)
    # ==================================================================
    def _get_bulletproof_spot(self, code: str):
        ...
        if self.tdx:
            ...
            try:
                quote = None
                if hasattr(self.tdx, 'get_real_quote'):
                    quote = self.tdx.get_real_quote(clean_code)
                elif hasattr(self.tdx, 'get_realtime_quote'):
                    quote = self.tdx.get_realtime_quote(clean_code)
                if quote and quote.get('price') and quote['price'] > 0:
                    p_tdx = {
                        ...
                        'prev_close': quote.get('last_close') or quote.get('prev_close'),
                    }
                    _merge(p_tdx, 'TDX-Binary')
                    success_tiers.append('TDX-Binary')
                    self._diag(f"P-1 通达信 ✓ price={p_tdx['price']} ({time.time()-t_tdx:.2f}s)")
            except Exception as e:
                self._diag(f"P-1 通达信跳过: {type(e).__name__}", "DEBUG")

        # ---------- P0: 东财直连 ----------
        t0 = time.time()
        try:
            proxies = _get_proxy_strategy()
            quote = _em_api.get_quote(code=clean_code, proxies=proxies, diag=self._diag)
            if quote and quote.get('price') and quote['price'] > 0:
                p0 = {
                    'price': quote['price'],
                    'open': quote.get('open') or 0,
                    ...
                    try:
                        ...
                        if df_xq is not None and not df_xq.empty:
                            ...
                            if p1['price'] > 0:
                                ...
                                self._diag(f"P1 雪球个股 ✓ price={p1['price']} high={p1['high']} "
                                          f"({time.time()-t0:.2f}s)")
                    except Exception as e:
                            ...
                            if code_col:
                                ...
                                if not target.empty:
                                    ...
                                    if p2['price'] > 0:
                                        ...
                                        self._diag(f"P2 新浪快照 ✓ price={p2['price']} "
                                                  f"({time.time()-t0:.2f}s)")
            except Exception as e:
                self._diag(f"P2 新浪快照异常: {type(e).__name__}", "WARN")
                            ...
                            if df_sina_min is not None and not df_sina_min.empty:
                                ...
                                if price_col:
                                    ...
                                    if not prices.empty:
                                        ...
                                        self._diag(f"P3 新浪今日分时 ✓ N={len(df_sina_min)} "
                                                  f"price={p3['price']} ({time.time()-t0:.2f}s)")
                        except Exception as e:
                            self._diag(f"P3 新浪分时异常: {type(e).__name__}", "WARN")
            except Exception:
                pass

        # ---------- P4: 腾讯日线 (补 prev_close 或兜底) ----------
        if not info.get('prev_close'):
            try:
                with bypass_proxy():
                    ...
                    if df_tx is not None and not df_tx.empty and len(df_tx) >= 2:
                        ...
                        if last_date == today_str:
                            ...
                            self._diag(f"P4 腾讯日线补充 prev_close={info.get('prev_close')}")
                ...
                self._diag(f"P4 腾讯日线异常: {type(e).__name__}", "WARN")

        # ---------- P5: akshare 东财快照 (最后兜底) ----------
        if os.environ.get("AK_ENABLE_EASTMONEY", "1") == "1" and not info:
            ...
            try:
                ...
                self._diag(f"P5 东财快照跳过", "DEBUG")

        if info and info.get('price', 0) > 0:
            defaults = {'price': 0, 'open': 0, 'high': 0, 'low': 0,
            ...
            self._diag(f"最终成功 source={info.get('source')} 今日={info['is_today']} "
                      f"命中={success_tiers}")
            return info
    ...
    def get_daily_kline(self, code: str, limit: int = 30, spot_row: dict = None):
        try:
            ...
            if self.tdx:
                try:
                    if hasattr(self.tdx, 'connect') and self.tdx.connect():
                        market_code = 1 if prefix == 'sh' else 0
                        bars = self.tdx.api.get_security_bars(4, market_code, clean_code, 0, max(limit, 60))
                        if bars:
    ...
    def get_intraday_plot_data(self, code: str) -> pd.DataFrame:
        ...
        if self.tdx:
            try:
                if hasattr(self.tdx, 'connect') and self.tdx.connect():
                    t_tdx = time.time()
                    market = 1 if prefix.lower() == 'sh' else 0
                    bars = self.tdx.api.get_security_bars(8, market, clean_code, 0, 240)
                    if bars:
                        df = pd.DataFrame(bars)
                        ...
                        if 'Volume' in df.columns:
                            ...
                            if df['Volume'].sum() > 0:
                                ...
                                df['AvgPrice'] = (cum_pv / cum_v).round(3)
                        # 只保留今日
                        today_df = df[df['datetime'].astype(str).str[:10] == today_str]
                        if not today_df.empty:
                            self._diag(f"分时来源: 通达信 (今日N={len(today_df)}) ✓ "
                                      f"({time.time()-t_tdx:.2f}s)")
                            cols = ['Time', 'Price'] + (['Volume'] if 'Volume' in today_df.columns else []) + \
                                   (['AvgPrice'] if 'AvgPrice' in today_df.columns else [])
                            return today_df[cols].reset_index(drop=True)
            except Exception as e:
                self._diag(f"分时 P-1 通达信跳过: {type(e).__name__}", "DEBUG")

        # P0: 东财直连
        try:
            proxies = _get_proxy_strategy()
            df = _em_api.get_intraday(code=clean_code, proxies=proxies, diag=self._diag)
            if df is not None and not df.empty:
        ...
        try:
            ...
            self._diag(f"分时 P1 新浪日内 失败: {type(e).__name__}", "WARN")
        ...
        try:
            ...
            self._diag(f"分时 P2 新浪分钟 失败: {type(e).__name__}", "WARN")
        ...
        if os.environ.get("AK_ENABLE_EASTMONEY", "1") == "1":
            try:
                ...
                self._diag(f"分时 P3 东财 失败: {type(e).__name__}", "WARN")
    ...
    def _intraday_to_text_samples(self, intraday_df: pd.DataFrame, step_minutes: int = 15) -> str:
        ...
        try:
            ...
            for _, r in sampled.iterrows():
                avg_part = f" | 均价 {r['AvgPrice']}" if 'AvgPrice' in r and pd.notna(r.get('AvgPrice')) else ""
                vol_part = f" | 量 {int(r['Volume'])}" if 'Volume' in r and pd.notna(r.get('Volume')) else ""
    ...
    def get_full_analysis_context(self, query: str):
        ...
        if has_spot:
            ...
            last_date = str(daily_df.iloc[-1]['日期'])
            ...
            ctx += "\n**分析指令**：\n"
            ctx += "- ✅ **可以基于下方【K线表】做完整的趋势/均线/形态/变盘点分析**。\n"
            ctx += "- ✅ 资讯面、板块、全市场情绪(本报告后续章节)若有数据均可正常使用。\n"
            if is_trading_hour:
                ctx += "- ⚠️ 仅限定：**禁止凭空给出今日盘中具体数值**。\n"
        ...
        if intraday_df.empty:
            if is_trading_hour:
                ctx += "分时数据暂时抓不到 —— 不影响基于 K 线的趋势结构分析。\n\n"
            else:
                ctx += "当前非交易时段，暂无分时数据。\n\n"
        else:
        ...
        with st.expander(title, expanded=False):
            ...
            if new_mode == "force":
                ...
                new_proxy = st.text_input(
                    ...
                    help="Clash 默认 http://127.0.0.1:7890",
                )
            ...
            if st.button(f"🔍 探测本地 Clash/Mihomo", key=f"clash_probe_{code}_{int(time.time())}"):
                ...
                if info['found']:
                    ...
                    st.info(
                        "把上面的规则粘贴到你的 Clash 配置文件 `rules:` 段最前面,"
                        "保存后重载配置即可。"
                    )
                else:
                    st.warning("未检测到本地 Clash/Mihomo。不影响主流程 — 雪球+新浪依然可用。")
    ...
    def get_industry_board_list(self) -> pd.DataFrame:
        ...
        try:
            with bypass_proxy():
                ...
                if df is not None and not df.empty:
                    return df.rename(columns={'name': '板块名称', 'code': '板块代码'})
        except Exception:
            pass
        return pd.DataFrame()
    ...
    def get_board_constituents(self, board_name: str, board_type: str = "行业板块") -> str:
        ...
        try:
            ...
            pass
        return "成分股抓取受限。AI 已转换逻辑: 基于板块指数成交活跃度做定向推理。"
        ...
        try:
            with bypass_proxy():
                ...
                if all_s is not None and not all_s.empty and '涨跌幅' in all_s.columns:
                    ...
                    ctx += (f"\n## 4. 全市场情绪背景\n"
                            f"- **上涨/下跌家数**: {u} / {d}\n"
                            f"- **说明**: 市场整体呈现{'强' if u > d else '弱'}度。")
        except Exception:
            pass
        return ctx

    # ==================================================================
    # 📉 功能复位区：行情中心、盘口异动、财务、龙虎榜等页面接口
    # ==================================================================
        ...
        try:
            with bypass_proxy():
                if pool_type == "涨停":
                    ...
                    if hasattr(ak, 'stock_zt_pool_sub_new_em'):
                        try:
                            return ak.stock_zt_pool_sub_new_em(date=today)
                        except Exception:
                            pass
                    if hasattr(ak, 'stock_zh_a_new_em'):
                        return ak.stock_zh_a_new_em()
        except Exception as e:
            self._diag(f"股池 {pool_type} 抓取失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()
    ...
    def get_market_changes(self, symbol: str = "大笔买入") -> pd.DataFrame:
        ...
        try:
            ...
            self._diag(f"盘口异动失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()
    ...
    def get_earnings_summary(self, date_str: str, report_type: str = "业绩快报") -> pd.DataFrame:
        ...
        try:
            ...
            self._diag(f"财务 {report_type} 失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()
    ...
    def get_institutional_research(self) -> pd.DataFrame:
        ...
        try:
            ...
            self._diag(f"机构调研失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()

    def get_global_news(self) -> pd.DataFrame:
        """7x24 快讯 — 6_📰_资讯中心.py 调用"""
        for fn_name in ['stock_info_global_em', 'stock_info_global_sina',
            ...
            try:
                ...
                continue
        self._diag("全球快讯: 所有源都失败", "WARN")
        return pd.DataFrame()

    def get_stock_news_detail(self, code: str) -> pd.DataFrame:
        """个股新闻(DataFrame) — 6_📰_资讯中心.py 调用"""
        clean_code = code.replace("sh", "").replace("sz", "")
        try:
            with bypass_proxy():
    ...
    def get_fund_flow_rank(self, indicator: str = "今日") -> pd.DataFrame:
        ...
        try:
            ...
            self._diag(f"资金流排行失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()
    ...
    def get_top_shareholders(self, code: str) -> pd.DataFrame:
        ...
        try:
            ...
            self._diag(f"十大股东失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()

    def get_shareholder_count_detail(self, code: str) -> pd.DataFrame:
        """股东户数趋势 — 8_👥_股东研究.py 调用"""
        clean_code = code.replace('sh', '').replace('sz', '')
        try:
            with bypass_proxy():
            ...
            self._diag(f"股东户数失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()
    ...
    def get_board_list(self, board_type: str = "行业") -> pd.DataFrame:
        ...
        try:
            ...
            self._diag(f"板块列表失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()
    ...
    def get_board_constituents_detail(self, board_name: str,
        ...
        try:
            ...
            self._diag(f"板块成分股失败: {type(e).__name__}", "WARN")
        return pd.DataFrame()