...
这是一款为你量身打造的 A 股决策中枢。系统不仅具备全市场实时行情抓取与多维数据深度剖析的能力，更首创了 **【个人经验注入】** 机制：你可以将自己的交易纪律与操盘铁律写给 AI，它将在每一次趋势推演和个股诊断中，无条件将你的策略置于最高优先级。无论是借助内置的专业量化模型，还是使用你专属的实战逻辑，它都能为你提供最敏锐的行情预测与风控警报。
...
> [!TIP]
> **💡 模型选择建议**：金融市场犹如没有硝烟的战场，盘面研判极其考验 AI 的逻辑推理深度。为了让本系统输出最敏锐、最准确的操作建议，**建议使用 Gemini-3.1、Claude-4.6 或 ChatGPT-5.4 等顶级推理模型**。
>
...
- **东财，雪球，新浪全挂时用的是腾讯日线** —— 这是昨收数据，不是今日实时（诊断面板会标注数据源）
